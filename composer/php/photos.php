...
//phpinfo();
//$link = mysqli_connect("localhost", "devuser", "devpass", "test_db");
...
set_time_limit(300);
    if (($handle = fopen("covid-chestxray-dataset/metadata.csv", "r")) !== FALSE) {


        while (($data = fgetcsv($handle)) !== FALSE) {

	if (stristr($myvar,$data[22])){
		if (empty($data[5])){
			$surv= "We still don't know";
		}else{
			$surv=$data[5];
		} 
		echo  "Finding: " .$data[4]."<br>";
		echo  "Age: " .$data[3]."<br>";
		echo  "Survival: " .$surv."<br>";
		echo  "Location: " .$data[20]."<br>";
		//echo  "Clinical notes: " .$data[26]."<br>";
	break;	
	}
        ...
        <?php
            ...
            echo '<div class="col-lg-2"><p class="alert alert-info">No Photos Found</p></div>';
        }
    ?>
</div> <!-- End row -->
